...
if DISC == 'trot':
    sys.path.insert(0, '/tmp/turf'); import features as F, dataset as DS, model, analyze as _
...
print(f"\n{'N':>3} {'Cheval':<20} {'cote':>6} {'P_gag':>7} {'±MC':>6} {'±modele':>14} "
      f"{'P_top3':>7} {'P_top5':>7} {'P_nonclasse':>11}")
recs = []
for i in np.argsort(-sim['p_win']):
    ...
    recs.append(dict(num=int(p['numPmu']), nom=p['nom'], cote=float(odds[i]),
                     ...
                     p_top3=float(sim['p_top3'][i]), p_top5=float(sim['p_top5'][i]),
